...
**EzBoost** is a modern, production-ready boosts plugin for Spigot / Paper / Bukkit 1.7–1.21.*. It delivers configurable potion effects with GUI activation, cooldown management, optional Vault costs, and world-based restrictions. It is a renewed take on [RedBull](https://dev.bukkit.org/projects/redbull).
...
- **Boost token items**: Give, trade, or reward boost tokens with `/ezboost give`.
...
| `/ezboost give <player> <boostKey> [amount]` | Give boost token items. | `ezboost.give` |
...
5. Grant per-boost permissions (like `ezboost.boost.speed`) to control access.
