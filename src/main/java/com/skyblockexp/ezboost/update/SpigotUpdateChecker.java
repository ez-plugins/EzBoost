...
public final class SpigotUpdateChecker {
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                ...
                if (!latestVersion.equalsIgnoreCase(currentVersion)) {
                    plugin.getLogger().info("A new EzBoost version is available: " + latestVersion
                            + " (current: " + currentVersion + "). Download: https://www.spigotmc.org/resources/"
                            + resourceId + "/");
                } else {
                    plugin.getLogger().info("EzBoost is up to date.");
                }
            } catch (Exception ex) {
                plugin.getLogger().warning("Failed to check for EzBoost updates: " + ex.getMessage());
            }
        });
    }
    ...
    private String fetchLatestVersion() throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "EzBoost Update Checker");
